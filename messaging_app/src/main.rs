...
async fn main() -> Result<()> {
    ...
    /*
    * Oppgave 1
    * Lag enums for commandoer. En enums av structs
    * Unwrap og Optional
    * Parse de
    * Skriv en match som returnerer en tuple med topic og peers
    * Hint?: topic.unwrap_or_else(|| TopicId::from_bytes(rand::random())); Ticket::from_str(ticket)?;
    *
#[derive(Parser, Debug)]
enum Command {
    /// Open a chat room for a topic and print a ticket for others to join.
    ...
    /// Join a chat room from a ticket.
    #1 Define a join command. Must include a ticket and a name
}
}
    ...
    let (topic, peers) = match &args.command {
        Command::Open => {
            ...
            (TopicId::from_bytes(rand::random()), vec![])
        }
        #1 Implement a join command
    };
    */

    /**** TODO ***/
    let (topic, peers) = get_topics_and_peers(&args.command);
